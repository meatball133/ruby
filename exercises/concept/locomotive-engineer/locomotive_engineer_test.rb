...
class LocomotiveEngineerTest < Minitest::Test
  def test_get_array_of_wagons_with_a_few_inputs
    assert_equal [1,5,2,7,4], LocomotiveEngineer.get_array_of_wagons(1,5,2,7,4)
  end

  def test_get_array_of_wagons_with_two_inputs
    assert_equal [1,5], LocomotiveEngineer.get_array_of_wagons(1,5)
  end

  def test_get_array_of_wagons_with_one_inputs
    assert_equal [1], LocomotiveEngineer.get_array_of_wagons(1)
  end

  def test_get_array_of_wagons_with_many_inputs
    assert_equal [1,10,6,3,9,8,4,14,24,7], LocomotiveEngineer.get_array_of_wagons(1,10,6,3,9,8,4,14,24,7)
  end
  ...
  def test_fix_array_of_wagons_many_inputs
    ...
    LocomotiveEngineer.fix_array_of_wagons([3, 27, 1, 14, 10, 4, 12, 6, 23, 17, 13, 22, 28, 19], [8, 10, 5, 9, 36, 7, 20])
  end

  def test_fix_array_of_wagons_the_inputs_are_same_length
    assert_equal [1, 8, 6, 15, 4, 2],
    LocomotiveEngineer.fix_array_of_wagons([4, 2, 1], [8, 6, 15])
  end

  def test_fix_array_of_wagons_the_missing_wagons_input_is_longer
    assert_equal [1, 8, 6, 4, 5, 9, 21, 2, 13, 25, 7, 19, 10, 3, 14],
    LocomotiveEngineer.fix_array_of_wagons([3, 14, 1, 25, 7, 19, 10], [8, 6, 4, 5, 9, 21, 2, 13])
  end
